...
start_background_task() {
    echo "$1"
    nohup python3 main.py --process val --path "$1" --cpus "0-23" --gpus "0" > "val.log" 2>&1 &

    sleep 10
}

wait_for_completion() {
    while pgrep -f "python3 main.py --process val --path $1 --cpus 0-23 --gpus 0" > /dev/null; do
        sleep 30
    done
}
...
task "$dir/psaresnet18_1"
task "$dir/psaresnet18_2"
task "$dir/psaresnet18_3"
task "$dir/psaresnet18_4"
